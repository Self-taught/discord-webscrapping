from update_user_data import update_gifts_received, update_gifts_sent, update_easter_egg_data, \
    update_golden_eggs_bought, update_raffle_points, update_golden_eggs_sent_daily, \
    update_reward_chest_bought, update_gift_iron, update_gift_fairy, \
    update_gift_beta_packs, update_gift_alpha_packs

update_easter_egg_data()
update_gifts_received()
update_gifts_sent()
update_golden_eggs_bought()
update_raffle_points()
update_golden_eggs_sent_daily()
update_reward_chest_bought()
update_gift_iron()
update_gift_fairy()
update_gift_beta_packs()
update_gift_alpha_packs()